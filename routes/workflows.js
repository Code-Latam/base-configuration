...
router.post("/register", async (request, res) => {
   ...
   if (!req.body.clientNr)
      ...
      if (myworkflow)
       {
        res.status(401).json(utils.Encryptresponse(req.encryptresponse,"A workflow object with this name allready exists for this client and explorer Id",req.body.apiPublicKey));
        return
       } 
...
const req = await utils.getDecodedBody(request);
  ...
  try {
    ...
    const workflow = await Workflow.findOneAndUpdate({ $and: [{ clientNr: req.body.clientNr }, { name: req.body.name }, { explorerId: req.body.explorerId }] }, {
    $set: req.body});
...
router.post("/delete", async (request, res) => {
  ...
  if (!req.gwokenPass)
     ...
     const myworkflow = await Workflow.findOne({ clientNr: req.body.clientNr, explorerId:req.body.explorerId, name: req.body.name })
      if (!myworkflow)
       {
        res.status(401).json(utils.Encryptresponse(req.encryptresponse,"A Workflow object with this clientNr, explorerId and name does not exist. Unable to delete",req.body.apiPublicKey));
        return
       } 
      
   try {
    var workflow = await Workflow.findOneAndDelete({ $and: [{ clientNr: req.body.clientNr }, { explorerId: req.body.explorerId }, { name: req.body.name }] });
    if (!workflow)
    {
...
router.post("/query", async (request, res) => {
  ...
  try {
    ...
    const workflow = await Workflow.findOne({ clientNr: req.body.clientNr, explorerId: req.body.explorerId,name: req.body.name });
    if (!workflow) {res.status(404).json(utils.Encryptresponse(req.encryptresponse,"No workflow object found for this clientNr, explorerId and name combination",req.body.apiPublicKey))}
...
router.post("/queryall", async (request, res) => {
   ...
   try {
     ...
     if (!workflows) {res.status(404).json(utils.Encryptresponse(req.encryptresponse,"No Workflow object found for this clientNr",req.body.apiPublicKey))}
 ...
 router.post("/querygraph", async (request, res) => {
   ...
   try {
    ...
    const fetchLinksForWorkflow = async (workflow) => {
      const links = await Link.findOne(
        { clientNr: req.body.clientNr, exploreId: req.body.explorerId, workflowName: workflow.name }
      );
      return links;
    };
